#!/usr/bin/env bash
# roboomp container entrypoint. Everything is baked into the image; we only
# sanity-check the runtime mount and create state dirs.
#
# TypeScript port of python/robomp/entrypoint.sh. Used by both the orchestrator
# (CMD: `robomp serve`) and the sibling gh-proxy (compose command:
# `robomp proxy serve`). The proxy role does NOT need a $PI_ROOT pi checkout —
# it never runs omp.
set -euo pipefail
...
# as $@ here (after tini --), so $1=robomp, $2=proxy is the canonical shape;
# we also accept a single concatenated arg for safety.
is_proxy_role=0
if [ "${1:-}" = "robomp" ] && [ "${2:-}" = "proxy" ]; then
    is_proxy_role=1
